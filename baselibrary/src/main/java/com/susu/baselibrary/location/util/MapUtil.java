...
import static com.susu.baselibrary.location.util.BitmapUtil.bmEnd;
...
public class MapUtil {
    ...
    public void setCenter() {
        LogUtils.d("test---", "CurrentLocation.latitude: " + CurrentLocation.latitude);
        LogUtils.d("test---", "CurrentLocation.longitude: " + CurrentLocation.longitude);
        ...
        String lastLocation = CommonUtil.getTrackConfig().getString(LocationConstants.LAST_LOCATION, null);
        LogUtils.d("test---", "lastLocation: " + lastLocation);
    ...
    public void updateStatus(LatLng currentPoint, boolean showMarker) {
        ...
        if (null != baiduMap.getProjection()) {
            ...
            if (screenPoint.y < 200 || screenPoint.y > DisplayUtils.getLcdHeight(CoreUtils.getApp()) - 500
                    || screenPoint.x < 200 || screenPoint.x > DisplayUtils.getLcdWidth(CoreUtils.getApp()) - 200
                    || null == mapStatus) {
    ...
    public void drawHistoryTrack(List<LatLng> points, SortType sortType) {
        ...
        if (points.size() == 1) {
            OverlayOptions startOptions = new MarkerOptions()
                    .position(points.get(0)).icon(BitmapUtil.bmStart)
