...
package com.susu.baselibrary.view.customerview.tickerviewtest;
...
class TickerColumnManager {
    ...
    private TickerCharacterList mCharacterList; //"0123456789"
    ...
    void setCharacterLists(String characterLists) {
        mCharacterList = new TickerCharacterList(characterLists);

        this.supportedCharacters = new HashSet<>();

        this.supportedCharacters.addAll(mCharacterList.getSupportedCharacters());

        // Update character lists in current columns
        for (TickerColumn tickerColumn : tickerColumns) {
            tickerColumn.setCharacterLists(mCharacterList);
        }
    }
            ...
            switch (actions[i]) {
                case LevenshteinUtils.ACTION_INSERT:
                    tickerColumns.add(columnIndex,
                            new TickerColumn(mCharacterList, metrics));
